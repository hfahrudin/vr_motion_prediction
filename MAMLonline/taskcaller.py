...
def taskcaller (filename, system_rate, k):
...
#    print('std dataset yaw', np.mean(ori_std[2]))
    ...
    x_seq, t_seq, x_test, t_test = train_test_split_k (x_seq, t_seq, k)
    x_seq = tf.convert_to_tensor(x_seq)
    t_seq = tf.convert_to_tensor(t_seq)
    x_test = tf.convert_to_tensor(x_test)
    t_test = tf.convert_to_tensor(t_test)
    
    return  x_seq, t_seq, x_test, t_test, input_nm, target_nm, data_length, DELAY_SIZE, train_eule_data, anticipation_size, train_time_data
